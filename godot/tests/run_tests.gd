...
func _test_battlefield_impact_tiers() -> void:
	...
	var light_preset: Dictionary = battlefield.impact_preset("light")
	var medium_preset: Dictionary = battlefield.impact_preset("medium")
	var heavy_preset: Dictionary = battlefield.impact_preset("heavy")
	...
	battlefield.play_events([{"type": "heavy_strike", "momentum_ratio": 1.0}])
	_expect(battlefield._hero_attack_tier == "heavy" and is_equal_approx(battlefield._hero_slash_duration, float(heavy_preset.action_duration)), "滿勢重擊必須使用重型動作節奏與揮刀音效")
