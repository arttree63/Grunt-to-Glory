...
const ENEMY_DEFS := {
	...
	"raider": {"name": "黑鐵劍兵", "role": "高頻攻擊", "hint": "出手快速，會頻繁養出格擋與閃避觸發", "hp": 0.9, "armor": 0.78, "damage": 0.78, "interval": 1.35, "behavior": {"movement": "rush", "preferred_distance": 84.0, "chase_speed": 132.0}},
	...
	"shield": {"name": "黑鐵盾衛", "role": "高護甲", "hint": "普通攻擊效率較低，破甲與魔劍更有效", "hp": 1.3, "armor": 2.45, "damage": 0.9, "interval": 2.65, "behavior": {"movement": "advance", "preferred_distance": 96.0, "chase_speed": 92.0}},
	"centurion": {"name": "黑鐵百夫長", "role": "精英統合", "hint": "重甲、蓄力重擊與低血狂暴的綜合考驗", "hp": 1.62, "armor": 1.75, "damage": 1.28, "interval": 2.45, "behavior": {"movement": "deliberate", "preferred_distance": 100.0, "chase_speed": 88.0}},
	...
	"boss": {"name": "黑鐵統領", "role": "重甲首領", "hint": "每隔數次攻擊施放碎甲重擊，30% 生命進入狂怒", "hp": 2.2, "armor": 1.5, "damage": 1.25, "interval": 2.0, "behavior": {"movement": "deliberate", "preferred_distance": 108.0, "chase_speed": 76.0}},
}
const ENEMY_ATTACK_CONTRACTS := {
	"normal": {"movement_lock": false, "targeting": "tracking", "shape": "contact", "recovery": 0.32},
	"heavy": {"movement_lock": true, "targeting": "locked_origin", "shape": "sector", "recovery": 0.62},
...
var spatial_danger_exposed := true
...
func set_spatial_combat_state(enabled: bool, enemy_distance: float, danger_distance := -1.0, danger_exposed := true) -> void:
	...
	spatial_danger_exposed = danger_exposed
...
func _spatial_attack_evaded(attack_type: String) -> bool:
	...
	if attack_type == "heavy":
		return not spatial_danger_exposed or spatial_danger_distance > SPATIAL_HEAVY_REACH
	if attack_type == "area":
...
func _defeat_hero() -> void:
	...
	var was_retrying := retry_pending
	var locked_retry_stage := retry_stage
	...
	stage = failed_stage if was_retrying else maxi(1, failed_stage - 1)
	current_wave = 0
	retry_pending = true
	retry_stage = locked_retry_stage if was_retrying else failed_stage
	...
	_spawn_enemy()
	_events.append({"type": "defeat", "failed_stage": retry_stage, "failed_wave": failed_wave, "fallback_stage": stage, "report": last_failure_report.duplicate(true), "during_recovery": was_retrying})
...
func _enemy_defeated() -> void:
	...
	var recovery_heal := _hero_max_hp() * 0.18 if retry_pending else (18.0 if journey_route == "village" else 10.0)
	hero_hp = minf(_hero_max_hp(), hero_hp + recovery_heal)
