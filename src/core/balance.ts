...
/**
 * 佔比平滑用的虛擬基礎點:share = (該科點數 + k) / (總點數 + 5k)。
 * ⚠️ 沒有它的話,買第一點會讓其他四科的佔比瞬間掉到 0(耐久上限掉四成、MP 回復掉四成),
 * 放置遊戲的第一顆正回饋反而變成「我變弱了」。平均分配在任何 k 下仍恰好是 0.2(×1.0)。
 */
export const TRACK_SHARE_SMOOTH = 3
...
/**
 * 信仰主屬性:全押時**每次觸發**回復耐久上限的比例(觸發式回血,與體能的持續回血分工)。
 * 觸發 = 擊殺 + 破盾 / 打斷蓄力 / 擊破圖騰(Boss 場也要有觸發點,見 faithHeal)。
 * ⚠️ 觸發頻率比想像高很多(高輸出時護盾層每 tick 都在破),0.05 會讓血條直接變裝飾。
 */
export const FAITH_HEAL_MAX = 0.015

// ── MP 制(v4.1 § 5,取代冷卻)──
/**
 * MP 池大小(抽象單位,不隨等級長——長的是回復速度)。
 * ⚠️ 必須裝得下**兩招**(二轉最貴的組合 360 + 320 = 680):
 * 只裝得下一招的話,v1.6 的「多槽疊窗(總攻)」結構性做不到——
 * 放完第一招要等 45 秒才付得起第二招,而第一招的視窗早就結束了。
 * 池子仍是「同時能放幾招」的上限,這是 MP 制與各自獨立冷卻最大的差別。
 */
export const MP_MAX = 800
/** 暫時技能實驗模式：全技能解鎖且 MP 不消耗，測完後關閉。 */
export const SKILL_LAB_MODE = true
/**
 * MP 每秒基礎回復。**刻意固定、不綁魔法科**——否則魔法會變成全職業必練的稅。
 * 技能的 MP 成本 = 它原本的冷卻秒數 × 這個值,所以平均分配的玩家施法頻率與改版前一致;
 * 魔法科的加成走「回復速度」(trackMult),做出「連續施放」的手感而不是撐久一點的爆發。
 */
export const MP_REGEN_BASE = 4
...
/** 戰場同時可維持的士兵上限。 */
/**
 * 同時可部署的士兵數。⚠️ 2026-08-02 由 5 改為 **6**:
 * 5 隻在「主角站中央」的構圖下**無法左右對稱**(必有一隻落在中線或某側多一隻),
 * 6 隻剛好 3+3。選 6 而不是 4 是因為先鋒號令的段數與威力都綁士兵數——
 * 砍到 4 等於大招弱化約 20%,加到 6 則是 +20%(這是刻意的,不是順帶)。
 */
export const ARMY_UNIT_MAX = 6
/** 每次主角自動攻擊命中累積的軍勢。 */
