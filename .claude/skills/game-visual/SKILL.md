...
### ✅ 主角揮擊已依此規格重做(2026-08-02)

改前 230ms 用**比例**切(預備只有 55ms、無命中幀、無頓格);
改後改用**絕對毫秒**的五拍表 `SWING_*_MS`(300ms 基準),疾風/盾擊等比縮放但命中拍有 20ms 下限。

| 拍 | 標準 | 改前 | 改後(實機逐幀量測) |
|---|---|---|---|
| 預備 | 100~200ms | 55ms | **120ms** ✅ |
| 命中定格 | 20~40ms | 無 | **30ms**,轉角定在峰值 0.34 不補間 ✅ |
| 頓格 | 1~2 幀 | 無 | **28ms(2 幀)**,`frame()` 直接 return ✅ |
| 收招 | 60~100ms | 97ms | 100ms ✅ |
| 擠壓拉伸 | — | 無 | 蓄力 1.056 → 出擊 0.92,**體積守恆** ✅ |
| 揮擊拖尾 | — | 無 | 4 道遞減弧光,120ms 淡出 ✅ |

⚠️ **實作時踩到的坑(下次會再踩,記著)**:`layoutHero()` 已經在寫 `heroBody.scale`
(那是隨螢幕的**基準縮放**)。擠壓拉伸第一版直接 `scale.set(1-squash, 1+squash)`
把基準蓋掉,**揮擊期間主角尺寸整個跑掉**。正解是存 `heroBaseScale` 再相乘,
並在揮擊結束的 early-return 分支把 scale 還原——那個分支原本只還原位置與旋轉。
又是一次「同一個值兩個算法」。

---

## 一之一、英雄技能 vs 軍團技能:演出職責分工(2026-08-02 裁決)

> **同一個演出不能同時承擔兩個職責。**

| | 英雄技能 | 軍團技能(號令) |
|---|---|---|
| 誰在動 | **英雄親自施放**,身先士卒 | **英雄不動**,只下令;衝的是士兵 |
| 驅動 | MP | **軍勢**,並消耗已部署士兵 |
| 效果 | 固定 | **依目前編制自動改變** |
| 例 | 衝鋒:英雄高速衝鋒 → 擊飛 → 重擊 | 先鋒號令:舉劍 → 軍旗亮起 → 先鋒軍衝鋒 |

⚠️ **這條規則是從一次實際失敗長出來的**:衝鋒與先鋒號令一度**共用同一個演出**
(士兵往前衝、多段命中),玩家的第一反應是「這不是同一招嗎」。
兩招在數值上差 2.5~3.15 倍,但**玩起來是同一件事**——能用計算機分高下的就不是決策。

**落地判準(新演出動工前逐條問)**:
1. 這招是英雄動還是軍團動?**不可以兩者都動。**
2. 如果是軍團技,效果有沒有隨編制改變?沒有的話它只是「換皮的英雄技」。
3. 這招的演出有沒有跟別招撞?撞了就先改身分,不要靠加大數字區分。
...
| 受擊閃白 | 命中確認 | ✅ 已有(70ms) |
| 命中粒子 | 力量的方向與量 | ✅ 已有 |
| **頓格 hit stop** | **打擊感的最大單一來源** | ✅ 已實作(揮擊)|
| **擠壓拉伸 squash & stretch** | 重量與彈性 | ✅ 已實作(揮擊);UI 尚未 |
| **殘影拖尾 trail** | 速度 | ✅ 已實作(揮擊)|
